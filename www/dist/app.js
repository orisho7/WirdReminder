...
function renderReaderContent(pagesData, fullLineInfoMap) {
  ...
  pagesData.forEach((pageData) => {
    ...
    const pageFullInfo = fullLineInfoMap
      ? fullLineInfoMap.get(pageNumber)
      : null;
      ...
      if (lineWords) {
        ...
        if (partialType !== "full" && pageFullInfo) {
          ...
          if (fullInfo && fullInfo.totalWords > 0) {
            ...
            if (
              partialType === "partial-start" ||
              partialType === "partial-both"
            ) {
              // Gap at start (right side in RTL)
              const startGap =
                (wirdFirstPos - fullInfo.firstPos) / fullInfo.totalWords;
              lineDiv.style.paddingRight = `${Math.round(startGap * 100)}%`;
            }
            if (
              partialType === "partial-end" ||
              partialType === "partial-both"
            ) {
              // Gap at end (left side in RTL)
              const endGap =
                (fullInfo.lastPos - wirdLastPos) / fullInfo.totalWords;
        ...
        if (nextLine <= 15) {
          ...
          if (surahId && gap === 2) {
            ...
            const alreadyAdded =
              prevIntro && prevIntro.querySelector(".basmala");
            if (!alreadyAdded) {
              const introDiv = document.createElement("div");
              introDiv.className = "surah-intro";
              const basmala = document.createElement("div");
...
function reflowPage(pageDiv) {
  ...
  while (changed && safety < maxPasses) {
    ...
    for (let i = 0; i < allLines.length; i++) {
      ...
      while (
        line.scrollWidth > line.clientWidth + 2 &&
