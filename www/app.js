            ...
            LocalNotifications.addListener('localNotificationActionPerformed', (notificationAction) => {
                ...
                if (reminderId) {
                    ...
                    setTimeout(async () => {
                        ...
                        const reminder = user_reminders.find(r => r.id === reminderId) || presets.find(r => r.id === reminderId);
                        if (reminder) {
                            openReader(reminder);
                        }
                    }, 500);
...
async function openReader(reminder) {
    ...
    document.getElementById('reader-title').textContent = reminder.name || reminder.description;

    readerView.style.display = 'flex';
    appContainer.style.display = 'none';

    readerContent.innerHTML = '<div class="loading-state"><div class="spinner"></div></div>';
    ...
    try {
        ...
        readerContent.innerHTML = '<div class="empty-state"><p class="empty-state-text" style="color: hsl(var(--destructive));">فشل تحميل الآيات</p></div>';
    }
...
async function restoreBookmark() {
    ...
    if (word) {
        ...
        setTimeout(() => word.scrollIntoView({ behavior: 'smooth', block: 'center' }), 100);
    }
}
...
function showCalendarModal(reminder) {
    ...
    document.getElementById('calendar-reminder-name').textContent = reminder.name || reminder.description;
    renderCalendar();
...
async function renderCalendar() {
    ...
    history.filter(h => h.reminderId === reminder.id).forEach(h => {
        const date = new Date(h.timestamp);
        completedDates.add(`${date.getFullYear()}-${date.getMonth()}-${date.getDate()}`);
    });
...
function initFormListeners() {
    ...
    reminderTypeSelect.addEventListener('change', () => {
        populateTargetSelect();
        ayahRangeGroup.style.display = reminderTypeSelect.value === 'ayah_range' ? 'block' : 'none';
    });

    frequencySelect.addEventListener('change', () => {
        daySelectGroup.style.display = frequencySelect.value === 'weekly' ? 'block' : 'none';
    });
    ...
    addForm.addEventListener('submit', async (e) => {
        ...
        if (type === 'ayah_range') {
            target.startAyah = parseInt(document.getElementById('start-ayah').value) || 1;
...
function initSettingsListeners() {
    ...
    importFile.addEventListener('change', async (e) => {
        ...
        try {
            ...
            if (data.user_reminders) await storage.set({ 'user_reminders': data.user_reminders });
            if (data.read_history) await storage.set({ 'read_history': data.read_history });
    ...
    document.getElementById('reset-extension').addEventListener('click', async () => {
        if (confirm('هل أنت متأكد من إعادة ضبط كل البيانات؟ لا يمكن التراجع عن هذه الخطوة.')) {
